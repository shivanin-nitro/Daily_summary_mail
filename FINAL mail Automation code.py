...
#CC_EMAILS = ["zodiac@getnitro.co","shivansh.s@getnitro.co","parijat@getnitro.co","shilpa@getnitro.co",,"shivani.n@getnitro.co"]
CC_EMAILS = ["shilpa@getnitro.co","shivani.n@getnitro.co"]
...
IMAGE_PATHS = {
    ...
    "fb": r"C:\Users\User1\Desktop\autmation codes\Logos\facebook icon.png",
    "li": r"C:\Users\User1\Desktop\autmation codes\Logos\linkdin icon.png",
    "tw": r"C:\Users\User1\Desktop\autmation codes\Logos\x icon.png",
    "ig": r"C:\Users\User1\Desktop\autmation codes\Logos\insta icon.png",
...
def run_pipeline():
    ...
    try:
        ...
        brand_name_lookup = {str(k).strip().upper(): str(v).strip() for k, v in zip(df_mapping['Brand'], df_mapping['name'])}
    ...
    for brand in ALL_BRANDS:
        ...
        # 1. Fetch as a list
        recipients = BRAND_EMAILS.get(brand.upper(), DEFAULT_RECIPIENT)
        if isinstance(recipients, str): recipients = [recipients]

        # 2. Join for the Email 'To' header
        to_header = ", ".join(recipients)
        
        # 3. Person Name Lookup
        person_name = brand_name_lookup.get(brand.upper())
        if not person_name:
            person_name = extract_name_from_email(recipients[0])
        ...
        if os.path.exists(temp_avail) and os.path.getsize(temp_avail) > 150:
            ...
            if not yesterday_df.empty:
                ...
                excel_df['dark store count'] = 1677
                excel_df['count availability %'] = 0 
        ...
        if os.path.exists(temp_sov) and os.path.getsize(temp_sov) > 150:
            ...
            if not df_s_target.empty:
                ...
                for col in ["overall", "organic", "ad"]: brand_comp[f"{col}_sov_%"] = (brand_comp[f"{col}_impressions"] / brand_comp[f"{col}_impressions"].sum()) * 100
        ...
        # ================= EMAIL SENDER / DRAFTER =================
        msg = MIMEMultipart('mixed')
        msg_related = MIMEMultipart('related')
        msg.attach(msg_related)
        msg['From'], msg['To'], msg['Cc'] = SENDER_EMAIL, to_header, ", ".join(CC_EMAILS)
        ...
        unsub_link = f"{WEB_APP_URL}?email={urllib.parse.quote(recipients[0])}&brand={urllib.parse.quote(brand)}"
            ...
            <div class="email-wrapper">
                ...
                <div class="footer">
                    <div class="social-icons" style="margin-bottom: 16px;">
                        <a href="{LINK_FB}"><img src="cid:fb"></a>
                        <a href="{LINK_LI}"><img src="cid:li"></a>
                        <a href="{LINK_TW}"><img src="cid:tw"></a>
                        <a href="{LINK_IG}"><img src="cid:ig"></a>
                        <a href="{LINK_WEB}"><img src="cid:web"></a>
                    </div>
                    <div style="font-size:13px; color:#475569; font-weight:600; margin-bottom:10px;">© 2026 Zodiac by Nitro</div>
                    <div style="font-size: 12px; color: #64748b;"> Don't want these emails?<a href="{unsub_link}" target="_blank" style="color: #475569;"> Unsubscribe</a></div>
                </div>
            </div>
        </body>
        </html>
        ...
        if len(missing_data) > 0:
            try:
                ...
                print(f"✅ Saved draft for {brand} (Missing: {', '.join(missing_data)})")
            except Exception as e: print(f"❌ Draft error for {brand}: {e}")
        else:
            try:
                ...
                print(f"✅ Email sent to {person_name} ({to_header}) for {brand}")
            except Exception as e: print(f"❌ Email error for {brand}: {e}")
